...
from perfana.core import correlation_measure, relative_price_index
...
def test_correlation_measure(duration, etf):
    bmk = etf.iloc[:, 0]
    portfolio = etf.iloc[:, 1:]

    def derive_expected():
        days = days_in_duration(duration)
        p = portfolio.pct_change()
        b = bmk.pct_change()
        return p.rolling(days).corr(b).dropna()

    actual = correlation_measure(portfolio, bmk, duration)
...
@pytest.mark.parametrize('duration', ['d', 'w', 'm', 'q', 's', 'y'])
def test_correlation_measure_multiple_benchmark(duration, etf):
    bmk = etf.iloc[:, :2]
